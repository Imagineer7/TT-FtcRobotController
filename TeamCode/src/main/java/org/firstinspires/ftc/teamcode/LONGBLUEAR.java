 ...
 * </pre>
 */
@Autonomous(name="LONG BLUE AR ", group="Timed Autonomous")
public class LONGBLUEAR extends LinearOpMode {
    ...
    private void runAutonomousSequence() {
        ...
        // If odometry fails, it will timeout and continue
        //Move back from goal zone
        moveForward(28, 0.2);
        sleep(300);

        // Move out of launch zone with 2 second timeout
        turnRelative(-18, 1.2);
        sleep(300);
        ...
        telemetry.update();
        fireMultipleShots(4, ShooterConfig.ShooterPreset.LONG_RANGE, false, 10.0);  // 8 second timeout
        sleep(500);
    ...
    public void fireMultipleShots(int numShots, ShooterConfig.ShooterPreset preset, boolean keepRunning, double timeoutSeconds) {
        ...
        // Wait for shooter to be ready with timeout
        while (opModeIsActive() && !emergencyStop && shootTimer.seconds() < timeoutSeconds) {
            if (robotManager.getShooterSystem().isShooterReady()) {
                break;
            }
            ...
            telemetry.addData("Timer", "%.1f / %.1f sec", shootTimer.seconds(), timeoutSeconds);
            telemetry.update();
            sleep(50);
        }
