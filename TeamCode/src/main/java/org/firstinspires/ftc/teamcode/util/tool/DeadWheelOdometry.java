...
/**
 * Dead Wheel Odometry wrapper for the GoBilda Pinpoint Odometry Computer
 * This class provides a unified interface for precise robot positioning using
 * the GoBilda Pinpoint system with IMU sensor fusion.
 */
public class DeadWheelOdometry {
     ...
     * @param hardwareMap Robot hardware map
     * @param telemetry Telemetry for debugging
     * @param xOffset X pod offset from center (mm, left is positive)
     * @param yOffset Y pod offset from center (mm, forward is positive)
     * @param podType Type of GoBilda pods being used
     */
    public DeadWheelOdometry(HardwareMap hardwareMap, Telemetry telemetry,
        ...
        try {
            pinpoint = hardwareMap.get(GoBildaPinpointDriver.class, "odo");
            configurePinpoint(xOffset, yOffset, podType);
            isInitialized = true;
            telemetry.addData("DeadWheelOdometry", "Initialized with custom config");
        } catch (Exception e) {
            telemetry.addData("DeadWheelOdometry", "Failed to initialize: " + e.getMessage());
            throw new RuntimeException("Failed to initialize DeadWheelOdometry", e);
        }
    ...
    public void updatePosition() {
        ...
        try {
            ...
            telemetry.addData("DeadWheelOdometry", "Update error: " + e.getMessage());
        }
    }
